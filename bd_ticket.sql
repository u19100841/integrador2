...
-- Procedimientos
--
CREATE PROCEDURE `listar_ticket` (IN `_id_area` INT, IN `_id_rol` INT, IN `_id_usuario` INT, IN `nro_ticket` varchar(100), IN `fecha_de` varchar(150), IN fecha_hasta varchar(150),
  IN `por_id_usuario` INT, IN `tipo_ticket` int, IN `estado_ticket` int  )
BEGIN
...
select t.id_ticket , t.titulo,t.descripcion ,t_ticket.nombre_tipo, 
        ...
        ((_id_rol in (2)) or _id_usuario = t.id_Soporte_ti) ) or _id_rol = 1) and 
        (nro_ticket='' or t.id_ticket=nro_ticket) and
        (fecha_de='' or t.fecha_registro between fecha_de and fecha_hasta) and
        (por_id_usuario=0 or t.id_Soporte_ti= por_id_usuario) and
        (tipo_ticket=0 or t.id_tipo= tipo_ticket) and
        (estado_ticket=0 or t.id_estado= estado_ticket) 
        
         order by t.id_ticket desc;
END$$

CREATE PROCEDURE `listar_ticket_por_cliente` (IN `_id_cliente` INT, IN `nro_ticket` varchar(100), IN `fecha_de` varchar(150), IN fecha_hasta varchar(150),
  IN `por_id_usuario` INT, IN `tipo_ticket` int, IN `estado_ticket` int)  BEGIN
...
select t.id_ticket , t.titulo,t.descripcion ,t_ticket.nombre_tipo, 
        ...
        left join areas a 
        on a.id_area = t.id_area
         where t.id_cliente = _id_cliente and
          (nro_ticket='' or t.id_ticket=nro_ticket) and
        (fecha_de='' or t.fecha_registro between fecha_de and fecha_hasta) and
        (por_id_usuario=0 or t.id_Soporte_ti= por_id_usuario) and
        (tipo_ticket=0 or t.id_tipo= tipo_ticket) and
